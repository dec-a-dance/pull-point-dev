...
then
	...
	if [ "$PREFIX" != "prod" -a "$PREFIX" != "preprod" ]
	then
...
echo "# *******************************************"
EXISTING_ID=( `docker ps -q --filter "label=project=$PROJECT" --filter "label=prefix=$PREFIX" --filter "name=$PROJECT-$PREFIX-$BASE_NAME"` )
